...
(function () {
  ...
  function rollPendingSalvo() { state.pendingSalvo = Math.random() < 0.5; }
  ...
  function buildShotProjectiles(side, slot, vx, vy, isCrit, useSalvo) {
  ...
  function fireProjectile(side, slot, vx, vy) {
    ...
    const useSalvo = !!state.pendingSalvo;
    let isCrit = false;
    if (isPlayerShot) {
      state.playerShotsFired += 1;
      isCrit = (state.playerShotsFired % PLAYER_CRIT_INTERVAL) === 0;
    }
    const volleyProjectiles = buildShotProjectiles(side, slot, vx, vy, isCrit, useSalvo);
    ...
    recoil(side, slot, isCrit, useSalvo);
  }
  function recoil(side, slot, isCrit, useSalvo) {
    const p = unitSlots[side][slot];
    const count = useSalvo ? (isCrit ? 18 : 12) : (isCrit ? 14 : 8);
    burst(state.particles, p.x, p.y - 8, "rgba(255,255,255,0.7)", count, 0.06);
    smoke(state.particles, p.x, p.y - 4, useSalvo ? (isCrit ? 12 : 8) : (isCrit ? 9 : 5));
    if (useSalvo) playSfx("shoot");
  ...
  function drawTrajectory() {
    ...
    const t = activeTurn();
    const previews = buildShotProjectiles(t.side, t.slot, vx, vy, critNext, !!state.pendingSalvo);
    ctx.save();
    const STEP = 96;
    const COUNT = 9;
